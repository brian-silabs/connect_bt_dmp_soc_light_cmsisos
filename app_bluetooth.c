...
static uint8_t ibeacon_advertising_set_handle = 0xff;
...
sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      ...
      bt_status = sl_bt_system_set_tx_power(SL_BT_CONFIG_MIN_TX_POWER, 0,
                                     &ret_power_min, &ret_power_max);
      app_assert_status_f(bt_status,
                          "[E: 0x%04x] Failed to set system power \n",
                          (int )bt_status);
...
static void bcn_setup_adv_beaconing(void)
{
  ...
  sc = sl_bt_advertiser_create_set(&ibeacon_advertising_set_handle);
  ...
  sc = sl_bt_legacy_advertiser_set_data(ibeacon_advertising_set_handle,
                                        0,
  ...
  sc = sl_bt_advertiser_set_timing(
      ibeacon_advertising_set_handle,
    140,     // min. adv. interval (milliseconds * 1.6)
    140,     // max. adv. interval (milliseconds * 1.6)
    0,       // adv. duration
    0);      // max. num. adv. events
  app_assert_status(sc);

  // Start advertising in user mode and disable connections.
  sc = sl_bt_legacy_advertiser_start(ibeacon_advertising_set_handle,
